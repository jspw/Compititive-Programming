...
int main(){
    ...
    // for(int i=0;i<26;i++)cout<<x[i]<<" "<<y[i]<<endl;
    for(int i=0;i<a.size();i++)x[int(a[i])-65]++;
    for(int i=0;i<b.size();i++)x[int(b[i])-65]++;
    for(int i=0;i<c.size();i++)y[int(c[i])-65]++;
    // for(int i=0;i<26;i++)cout<<x[i]<<" "<<y[i]<<endl;
    for(int i=0;i<26;i++)if(x[i]!=y[i]){cout<<"NO"<<endl;return 0;}
    cout<<"YES"<<endl;
